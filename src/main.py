...
def main():
    ...
    parser.add_argument('-x', '--xrange', nargs=2, type=float, default=XRANGE, help=f'Two-entry tuple that defines the domain of the graph. Default is {XRANGE}.')
    parser.add_argument('-y', '--yrange', nargs=2, type=float, default=YRANGE, help=f'Two-entry tuple that defines the range of the graph. Default is {YRANGE}.')
    ...
    if args.type == 'function':
        ...
        fig.build_axes()
        fig.load_data(os.path.join(script_dir,"data.csv"))
        print(fig)
    elif args.type == 'list':
